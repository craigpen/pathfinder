...
function renderDiscoverySelectorOptions() {
  ...
  // Define which selectors use pickN (multi-select) vs pick1 (single-select)
  const multiSelect = ['motivations', 'citizen', 'languages', 'flex'];
  ...
  Object.entries(opts).forEach(([qKey, options]) => {
    ...
    options.forEach(value => {
      let onclick = `${pickFunc}('${qKey}',this)`;
      // Special case: state selector should also trigger university filter
...
function renderDiscoveryPills() {
  // Sync multi-select pill states
  document.querySelectorAll('[data-q="motivations"] .pill').forEach(p => {
    if (S.motivations && S.motivations.includes(p.textContent))
      p.classList.add('on');
    else
  ...
  document.querySelectorAll('[data-q="flex"] .pill').forEach(p => {
    if (S.flex && S.flex.includes(p.textContent))
      p.classList.add('on');
    else
      p.classList.remove('on');
  });
  ...
  document.querySelectorAll('[data-q="citizen"] .pill').forEach(p => {
    if (S.citizen && S.citizen.includes(p.textContent))
      p.classList.add('on');
    else
      p.classList.remove('on');
  });
  ...
  document.querySelectorAll('[data-q="languages"] .pill').forEach(p => {
    if (S.languages && S.languages.includes(p.textContent))
      p.classList.add('on');
    else
      p.classList.remove('on');
  });

  // Sync single-select pill states
  document.querySelectorAll('[data-q="stateOfResidency"] .pill').forEach(p => {
    if (p.textContent === S.stateOfResidency)
      p.classList.add('on');
    else
      p.classList.remove('on');
  });
  ...
  if (S.scholarshipAmount) {
    document.getElementById('sl-schol').value = S.scholarshipAmount;
    document.getElementById('sl-schol-v').textContent = '$' + parseInt(S.scholarshipAmount).toLocaleString();
  }
  if (S.collegeSavings) {
    document.getElementById('sl-savings').value = S.collegeSavings;
    document.getElementById('sl-savings-v').textContent = '$' + parseInt(S.collegeSavings).toLocaleString();
  }
  if (S.partTimeWork) {
    document.getElementById('sl-work').value = S.partTimeWork;
    document.getElementById('sl-work-v').textContent = '$' + parseInt(S.partTimeWork).toLocaleString() + '/yr';
  }
  if (S.familySupport) {
    document.getElementById('sl-parent').value = S.familySupport;
    document.getElementById('sl-parent-v').textContent = '$' + parseInt(S.familySupport).toLocaleString() + '/yr';
  }
  if (S.loanRate) {
    document.getElementById('sl-rate').value = S.loanRate;
    document.getElementById('sl-rate-v').textContent = S.loanRate + '%';
  }
  if (S.loanRepaymentYears) {
    document.getElementById('sl-yrs').value = S.loanRepaymentYears;
    document.getElementById('sl-yrs-v').textContent = S.loanRepaymentYears + ' yrs';
  }
