...
window.S = {
  ...
  languages: [],
  postGrad: null,
  inStateTuitionPref: null,
  stateOfResidency: null,
  inStateToggle: false,
  usUniversityType: null,
  uniSelectivity: [],
...
function saveState() {
  const stateToSave = {
    ...
    postGrad: S.postGrad,
    inStateTuitionPref: S.inStateTuitionPref,
    stateOfResidency: S.stateOfResidency,
    cc: S.cc,
    costCC: S.costCC,
    expl: S.expl,
    uniSelectivity: S.uniSelectivity,
    scholarshipAmount: S.scholarshipAmount,
    collegeSavings: S.collegeSavings,
    partTimeWork: S.partTimeWork,
    familySupport: S.familySupport,
    loanRate: S.loanRate,
    loanRepaymentYears: S.loanRepaymentYears,
    inStateToggle: S.inStateToggle,
    usUniversityType: S.usUniversityType
  };
  // console.log('💾 Saving state:', stateToSave);
  localStorage.setItem('univPathfinderState', JSON.stringify(stateToSave));
}
function loadState() {
  const saved = localStorage.getItem('univPathfinderState');
  ...
  if (saved) {
    ...
    if (state.scholarshipAmount !== undefined) document.getElementById('sl-schol').value = state.scholarshipAmount;
    if (state.collegeSavings !== undefined) document.getElementById('sl-savings').value = state.collegeSavings;
    if (state.partTimeWork !== undefined) document.getElementById('sl-work').value = state.partTimeWork;
    if (state.familySupport !== undefined) document.getElementById('sl-parent').value = state.familySupport;
    if (state.loanRate !== undefined) document.getElementById('sl-rate').value = state.loanRate;
    if (state.loanRepaymentYears !== undefined) document.getElementById('sl-yrs').value = state.loanRepaymentYears;

    // Restore inStateToggle button state
    if (state.inStateToggle !== undefined) {
      const instateIn = document.getElementById('instate-in');
      const instateOut = document.getElementById('instate-out');
      if (state.inStateToggle) {
        instateIn.classList.add('on');
        instateOut.classList.remove('on');
      } else {
        instateOut.classList.add('on');
        instateIn.classList.remove('on');
      }
    }

    // Restore pill states for multi-select selectors
    if (S.uniSelectivity && S.uniSelectivity.length > 0) {
      document.querySelectorAll('[data-q="uniSelectivity"] .pill').forEach(p => {
        S.uniSelectivity.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.motivations && S.motivations.length > 0) {
      document.querySelectorAll('[data-q="motivations"] .pill').forEach(p => {
        S.motivations.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.languages && S.languages.length > 0) {
      document.querySelectorAll('[data-q="languages"] .pill').forEach(p => {
        S.languages.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.citizen && S.citizen.length > 0) {
      document.querySelectorAll('[data-q="citizen"] .pill').forEach(p => {
        S.citizen.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.flex && S.flex.length > 0) {
      document.querySelectorAll('[data-q="flex"] .pill').forEach(p => {
        S.flex.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
      });
    }

    // Restore pill states for single-select selectors
    if (S.vision) {
      document.querySelectorAll('[data-q="vision"] .pill').forEach(p => {
        S.vision === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.lang) {
      document.querySelectorAll('[data-q="lang"] .pill').forEach(p => {
        S.lang === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.cost) {
      document.querySelectorAll('[data-q="cost"] .pill').forEach(p => {
        S.cost === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.debtYrs) {
      document.querySelectorAll('[data-q="debtYrs"] .pill').forEach(p => {
        S.debtYrs === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.postGrad) {
      document.querySelectorAll('[data-q="postGrad"] .pill').forEach(p => {
        S.postGrad === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
    if (S.stateOfResidency) {
      document.querySelectorAll('[data-q="stateOfResidency"] .pill').forEach(p => {
        S.stateOfResidency === p.textContent ? p.classList.add('on') : p.classList.remove('on');
      });
    }
  }
}
...
function pick1(q, el) {
  // Remove 'on' class from all pills for this selector
  document.querySelectorAll(`[data-q="${q}"] .pill`).forEach(o => o.classList.remove('on'));
  el.classList.add('on');

  // Update state based on selector type
  if (q === 'citizen') {
    S.citizen = [el.textContent];
  } else if (q === 'flex') {
    S.flex = el.textContent;
  } else if (q === 'cost') {
    S.cost = el.textContent;
  } else if (q === 'vision') {
    S.vision = el.textContent;
  } else if (q === 'lang') {
    S.lang = el.textContent;
  } else if (q === 'debtYrs') {
    S.debtYrs = el.textContent;
  } else if (q === 'postGrad') {
    S.postGrad = el.textContent;
  } else if (q === 'inStateTuitionPref') {
    S.inStateTuitionPref = el.textContent;
    if (document.getElementById('cost-table')) renderCostChips();
  } else if (q === 'stateOfResidency') {
    S.stateOfResidency = el.textContent;
    S.inStateTuitionPref = 'in-state-public';
    if (document.getElementById('uni-content')) renderUniversities();
    if (document.getElementById('cost-table')) renderCostChips();
  }
...
function pickN(q, el) {
  ...
  // Update state based on selector type (multi-select)
  if (q === 'motivations') {
    S.motivations = [];
    document.querySelectorAll(`[data-q="${q}"] .pill.on`).forEach(p => S.motivations.push(p.textContent));
  } else if (q === 'citizen') {
    S.citizen = [];
    document.querySelectorAll(`[data-q="${q}"] .pill.on`).forEach(p => S.citizen.push(p.textContent));
  } else if (q === 'flex') {
    S.flex = [];
    document.querySelectorAll(`[data-q="${q}"] .pill.on`).forEach(p => S.flex.push(p.textContent));
  } else if (q === 'languages') {
    S.languages = [];
    document.querySelectorAll(`[data-q="${q}"] .pill.on`).forEach(p => S.languages.push(p.textContent));
  } else if (q === 'uniSelectivity') {
    S.uniSelectivity = [];
    document.querySelectorAll(`[data-q="${q}"] .pill.on`).forEach(p => S.uniSelectivity.push(p.textContent));
    document.querySelectorAll(`[data-q="${q}"] .pill`).forEach(p => {
      S.uniSelectivity.includes(p.textContent) ? p.classList.add('on') : p.classList.remove('on');
    });
    if (document.getElementById('uni-content')) renderUniversities();
  }
